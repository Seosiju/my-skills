...
- **v4 (2026-07-04)** — 구현 결과 리뷰 반영(사후 승인 기록).
  - T1 의도적 변경 표에 `share --plan` risks 중복 제거를 추가 등재.
    커밋 `dac42bf`에 신고 없이 포함됐던 동작 변경을 검토 후 의도적 변경으로
    승인 — 같은 발견이 compose·audit 경로로 2회 표시되던 T1의 부작용 정리.
  - 범위 밖 리팩터링 3건(`128d1fc` audit JSON 타입화, `dac42bf` 명령 헬퍼
    모듈 분할, `9759ee4` 테스트 스위트 분할)은 구현자 노트 ①(범위 밖 금지)
    위반이나, 전체 테스트 통과와 동작 동일성 확인 후 **유지**로 결정.
  - T1이 요구한 CHANGELOG 기재 이행: `CHANGELOG.md` 신설(Unreleased 섹션에
    의도적 강화 3건 + share 중복 제거 + ignore 정책 단일화 기재).
...
- **결정: 정확한 동작 보존이 아니라 의도적 강화를 채택한다.** 규칙별 커스텀
  ...
  CHANGELOG에 기재한다. — 기재 완료: `CHANGELOG.md` Unreleased 섹션(v4).
  ...
  | `share --plan` risks 중복 표시 | 같은 발견이 compose·audit 항목으로 2회 표시 | file+message 기준 중복 제거로 1회 표시 (커밋 `dac42bf`, v4 사후 등재) |
