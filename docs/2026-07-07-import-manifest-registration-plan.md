...
### F7. `cmd_import`는 effective 매니페스트와 main 매니페스트를 구분해야 한다
`load_manifest`가 돌려주는 `Manifest.skills`는 `dict[str, Skill]`이고
각 `Skill`에 `enabled`/`hosts`가 있다 (`config.py:62-79`). 다만 이 값은
main `my-skills.toml`과 `my-skills.local.toml`이 병합된 effective view다.
따라서 "이 스킬이 main manifest에 등록되어 있는가?"는
`name in manifest.skills`가 아니라 main 파일의 `[skills.<name>]` section
존재 여부로 판별해야 한다.
...
구워지는 부작용이 생긴다. 반대로 local에만 있는 스킬을 effective view
때문에 "등록됨"으로 착각하면 main manifest에 section을 만들지 못한다.
그러므로 "기존 상태 보존"은 **main manifest section이 이미 있을 때만**
재등록을 건너뛰는 방식이어야 한다 (§5.1).
...
2. **기존 항목 보호 (멱등성) — 확정 (2026-07-08 교차 검토 반영):**
   ...
   없이 꺼지는 사고를 막아야 한다. main manifest section 존재 여부를
   기준으로 분기한다 (F7):

```python
if not has_skill_section(root / "my-skills.toml", name):
...
```
   ...
   - 기각된 대안: `register_skill(preserve_enabled=...)` 파라미터 추가 —
     등록 여부 판별과 쓰기 동작을 섞어 API 표면을 늘린다.
...
4c. **local 오버레이 미오염 (F7):** `my-skills.local.toml`로 enabled가
   ...
   바이트 단위로 불변.
4d. **local-only 오버레이 등록 (F7):** 스킬이 `my-skills.local.toml`에만
   있고 main `my-skills.toml`에는 없으면, import가 main manifest에 새
   `[skills.<name>]` 섹션을 만든다. `--enable`도 같은 경로에서 동작한다.
5. **up-to-date 경로 등록 (F5):** canonical과 동일한 소스를 import했고
   매니페스트에 미등록이면, 등록이 수행된다.
6. **audit/validation 차단 시 미등록:** 기존 차단 경로에서 매니페스트가
   변경되지 않는다.
7. **symlink 소스 차단:** import source 자체 또는 하위 항목이 symlink면
   복사·등록 전에 차단하고, 매니페스트와 canonical 디렉터리를 변경하지 않는다.
8. **skills 목록 노출:** import 후 `cmd_skills` 출력에 스킬이 나타난다.
...
- [ ] 재-import가 기존 항목의 `hosts`를 바꾸지 않고, local 오버레이 값을
      메인 TOML에 되쓰지 않는다.
- [ ] local-only 오버레이에만 있는 스킬도 main `my-skills.toml`에 등록된다.
- [ ] symlinked import source는 복사·등록 전에 차단되고 매니페스트를
      변경하지 않는다.
...
- **2026-07-08** 리뷰 보강 (Codex): local-only overlay가 effective
  manifest에는 보이지만 main manifest에는 없는 결함을 반영해 F7과 §5.1을
  main manifest section 기준 판별로 정정하고, validation-blocked import
  manifest 불변 수용 기준을 명시적으로 증거화.
- **2026-07-08** 보안 리뷰 보강 (Codex): symlinked import source가 audit
  경계 밖 파일을 canonical registry로 복사할 수 있는 위험을 차단하고,
  내부 symlink 및 source-root symlink 회귀 테스트를 추가했다.
